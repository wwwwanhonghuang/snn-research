...
class CurrentBasedLIFNeuron(AbstractNeuron):
    def __init__(self, N, hyperparameters):
        ...
        self.set_states([None, self.vreset * np.ones(N), 0, 0, None])
    ...
    def update_state(self, u):
        ...
        tcount += 1
        self.set_states([s, v, tlast, tcount, last_fired_v])
        return self.states
 
    def reset_states(self, random_state = False ):
        if random_state:
            self.states[self.INDEX_V] = self.vreset + np.random.rand(self.N) * (self.vthr - self.vreset)
        else:
            self.states[self.INDEX_V] = self.vreset * np.ones(self.N)
            self.states[self.INDEX_TLAST] = 0
            self.states[self.INDEX_TCOUNT] = 0        
...
class ConductanceBasedLIFNeuron(AbstractNeuron):
    def __init__(self, N, hyperparameters):
        ...
        self.set_states([None, self.vreset * np.ones(N), None, 0, 0])
    ...
    def update_state(self, u):
        ...
        self.set_states([s, v, v_, tlast, tcount])
                
        return self.states
    
    def reset_states(self, random_state=False):
        if random_state:
            self.states[self.INDEX_V] = self.vreset + np.random.rand(self.N) * (self.states[self.vthr] - self.vreset)
        else:
            self.states[self.INDEX_V] = self.vreset * np.ones(self.N)
        self.states[self.INDEX_TLAST] = 0
        self.states[self.INDEX_TCOUNT] = 0
...
class DiehlAndCook2015LIFNeuron(AbstractNeuron):
    def __init__(self, N, hyperparameters):         
        ...
        self.set_states([None, self.vreset * np.ones(N), self.init_vthr, None, 0, 0, None])
    ...
    def reset_states(self, random_state=False): 
        if random_state: 
            self.states[self.INDEX_V] = self.vreset + np.random.rand(self.N) * (self.states[self.INDEX_VTHR] - self.vreset) 
        else: 
            self.states[self.INDEX_V] = self.vreset * np.ones(self.N)
            self.states[self.INDEX_VTHR] = self.init_vthr
            self.states[self.INDEX_THETA] = np.zeros(self.N) 
            self.states[self.INDEX_TLAST] = 0 
            self.states[self.INDEX_TCOUNT] = 0 
    ...
    def update_state(self, u):
        ...
        self.set_states([s, v, vthr, v_, tlast, tcount, theta]) 
        return self.states
